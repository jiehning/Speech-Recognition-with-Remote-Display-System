...
void ssd1306_send_cmd(ssd1306_t *p, uint8_t cmd) {
    ...
    i2c_write_blocking(p->i2c_i, p->address, cmd_buffer, 2, false);
}
...
int ssd1306_init(ssd1306_t *p, uint16_t width, uint16_t height, uint8_t address, i2c_inst_t *i2c_instance) {
	...
	p->buffer = malloc(p->bufsize+1); //keep one byte for the control command
	if (p->buffer == NULL){
		return false;
	}
	...
	ssd1306_send_cmd(p, SET_DISP_SATRT_LINE);
	ssd1306_send_cmd(p, 0x00);
...
void ssd1306_draw_char_with_font(ssd1306_t *p, uint32_t x, uint32_t y, const uint8_t *font, char c) {
    ...
    */
    ...
    for(uint8_t col=0; col<font[1]; col++) { // width
        ...
        //第0列(前五個資訊)+從32數來第幾個字元*字型寬度*高度幾個byte + 第幾個col的
